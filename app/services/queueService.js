  ...
  async create(userId, payload) {
    try {
      ...
      if (validateQueueInputs(payload)) {
        ...
        const item = await this.getSingleQueue(userId, result.id);
        ...
        // email text
        emailText += `QR Code (Web – scan using Google Lens, WhatsApp, or Camera): Attached below\n`;
        emailText += `Direct Link: ${webJoinUrl}\n\n`;
